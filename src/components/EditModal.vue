<template>
    ...
    <div class="modal" v-if="isOpen" role="dialog"
  ...
  try {
    ...
    if (this.isEditMode) {
      const response = await axios.put(`http://127.0.0.1:3000/user/updateTable/${this.rowData.id}`, this.formData);
      ...
      const response = await axios.post('http://127.0.0.1:3000/user/insertTable', this.formData);
      if (response.data.message === 'success') {
